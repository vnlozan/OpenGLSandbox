...
public:
	...
	// rewrite
	void DrawElements( const _VertexArray& va, const IndexBuffer& ib, const Shader& shader ) const;
	...
	void DrawArraysInstanced( const _VertexArray& va, const Shader& shader, unsigned int type = GL_TRIANGLES, int first = 0, int count = 0, int instancesCount = 2 ) const;
};