...
class _Model {
public:
    _Model( std::string const& path, bool gamma=false ): m_GammaCorrection{ gamma } {
...
private:
    ...
    _Mesh processMesh( aiMesh* mesh, const aiScene* scene ) {
        ...
        std::vector<_Texture2D> textures;
        ...
        loadMaterialTextures( material, aiTextureType_DIFFUSE, _Texture2D::TYPE::DIFFUSE, textures );
        loadMaterialTextures( material, aiTextureType_SPECULAR, _Texture2D::TYPE::SPECULAR, textures );
        loadMaterialTextures( material, aiTextureType_HEIGHT, _Texture2D::TYPE::HEIGHT, textures );
        loadMaterialTextures( material, aiTextureType_AMBIENT, _Texture2D::TYPE::AMBIENT, textures );

        //std::cout << "TEXTURES SIZE = " << textures.size() << std::endl;
        //std::cout << "TEXTURES_Loaded SIZE = " << m_TexturesLoaded.size() << std::endl;
        return _Mesh( vertices, indices, textures );
    }
    ...
    void loadMaterialTextures( aiMaterial* mat, aiTextureType aiType, _Texture2D::TYPE type, std::vector<_Texture2D>& outTextures ) {
        for( unsigned int i = 0; i < mat->GetTextureCount( aiType ); i++ ) {
            ...
            if( m_TLoaded.count( str.C_Str() ) > 0 ) {
                ...
                _Texture2D* texture = new _Texture2D{ fileName.c_str(), type, true };
...
private:
    std::vector<_Texture2D>     m_TexturesLoaded;
    std::string                 m_Directory;
    bool                        m_GammaCorrection;

    std::unordered_map<std::string, _Texture2D*> m_TLoaded;
};